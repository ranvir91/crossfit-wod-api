...
// get all existing workout from database
...
// get an existing workout from database
...
const createNewWorkout = (req, res) => {
    const {name, mode, exercises, equipment, trainerTips} = req.body;
    if(!name || !mode || !exercises || !equipment || !trainerTips) {
        res.status(400).send({
          status: "FAILED",
          data: {
            error:
    ...
    const newWorkout = {
        name, mode, exercises, equipment, trainerTips
    }
    try {
